...
//this whole validation thing really needs some best practice love
    ...
    } else return $validatedInput; //aka when it's not valid
    ...
    return array('isValidSubmission' => true, 'issueID' => $issueID);
}

//TODO: change to reflect the fact that if you have a visitlabel input then you don't need to insert a session
function validateInput($validateValues, $issueID)
{
    ...
    $old = null;
    if ($issueID) {
        $old = $db->pSelect("SELECT c.PSCID, s.Visit_label from candidate c ".
    ...
    if ((isset($validateValues['visitLabel']) && $oldPSCID)
        ...
        $isValidSession = $db->pSelectOne("SELECT s.ID FROM candidate c ".
