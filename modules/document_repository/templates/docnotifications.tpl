    ...
    {foreach from=$notifications item=link}
        <a href="/document_repository/Files/{$link.Data_dir}"
           download="{$link.File_name}" class="list-group-item">
            ...
            {/if}
            <span class="pull-right text-muted small">{dgettext("loris", "Uploaded")}: {$link.Date_uploaded}</span>
            <br>
            {$link.File_name}
        </a>
    {/foreach}
...
<!-- /.list-group -->
<a href="{$baseURL}/document_repository/" class="btn btn-default btn-block">{dgettext("document_repository", "Document Repository")}
