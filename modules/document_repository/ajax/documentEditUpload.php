    ...
    if ($action == 'upload') {
        ...
        if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_path)) {
            $success = $DB->insert(
                ...
                array(
                 ...
                 'File_type'     => $fileType,
                            ...
                            $msg_data['study']       = $config->getSetting('title');// @codingStandardsIgnoreLine
            ...
            foreach ($Doc_Repo_Notification_Emails as $email) {
                Email::send($email['Email'], 'document_repository.tpl', $msg_data);// @codingStandardsIgnoreLine
            }
        ...
        $fileName = $DB->pselectOne(
            "select File_name from document_repository where record_id=:record_id",// @codingStandardsIgnoreLine
            array('record_id' => $id)
        );
