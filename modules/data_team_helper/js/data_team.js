...
$(document).ready(
    function() {
        ...
        $('#autocomplete-ajax').devbridgeAutocomplete(
            {
                serviceUrl: loris.BaseURL + "/data_team_helper/ajax/GetCandidates.php",
                params:{
                    'instrument': function(){
        ...
        $("[name=bvl_feedback]").click(
            function(e) {
                ...
                var instrument_window = window.open(
                    loris.BaseURL + "/" + $candID + "/" + $sessionID + "/" + $test_name + "/"
                        + "&commentID=" + $commentID
                );
                $(instrument_window.document).ready(
                    function (e){
