    ...
    if (is_numeric($key)) {
        ...
        if ($value == "") {
            ...
            if (in_array($key, $pathIDs)) {
                ...
                if (!validPath($value)) {
                    $err = 'Directory `'
                        . htmlspecialchars($value, ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML5, null, false)
                        . '` is invalid';
        ...
        if ($action == 'add') {
            ...
            if (in_array($ConfigSettingsID, $pathIDs)) {
                if (!validPath($value)) {
                    $err = 'Directory `' . htmlspecialchars($value, ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML5, null, false)
                        . '` is invalid';
                    // Set response code and display error message.
                    displayError(400, $err);
                    return;
