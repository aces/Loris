...
<p>{dgettext('configuration', 'Use this page to manage the configuration of existing projects, or to add a new one.')}</p>
<p>{dgettext('configuration', 'To configure study cohorts')} <a href="{$baseurl|default}/configuration/cohort/">{dgettext('configuration', 'click here')}</a>.</p>

<div class="col-md-3">
<ul class="nav nav-pills nav-stacked" role="tablist" data-tabs="tabs">
    <li class="active"><a id="#projectnew{$ProjectID|default}" href="#projectnew" data-toggle="tab" class="active">{dgettext('configuration', 'New ProjectID')}</a></li>
...
<div class="col-md-7">
                ...
                <input type="hidden" name="ProjectID" value="{$ProjectID}" class="ProjectID">
                    ...
                    <div class="form-group">
                        <div class="col-sm-12 col-md-3" data-toggle="tooltip" data-placement="right" title="{'These cohorts will be automatically displayed for any candidate affiliated with this project at timepoint creation.'}">
                            <label class="col-sm-12 control-label">{dgettext('configuration', 'Affiliated Cohorts')}</label>
                        </div>
                        <div class="col-sm-12 col-md-9">
                    ...
                    <div class="form-group">
                        <div class="col-sm-offset-3 col-sm-9">
                            <button id="saveproject{$ProjectID}" class="btn btn-primary saveproject submit-area">{dgettext('loris', 'Save')}</button>
                            <button class="btn btn-default submit-area" type="reset">{dgettext('loris', 'Reset')}</button>
    ...
    <div id="projectnew" class="tab-pane active">
        <h2>{dgettext('configuration', 'New Project')}</h2>
        <br>
        <form class="form-horizontal" role="form" method="post" id="form{$ProjectID}">
            <fieldset>
                ...
                <div class="form-group">
                    <div class="col-sm-offset-3 col-sm-9">
                        <button id="saveprojectnew" class="btn btn-primary saveproject submit-area">{dgettext('loris', 'Save')}</button>
                        <button class="btn btn-default submit-area" type="reset">{dgettext('loris', 'Reset')}</button>
                        <label class="saveStatus"></label>
                    </div>
                </div>
