...
$QueryCondition = "$Field IS NOT NULL AND $FieldX IS NOT NULL AND c.Active='Y'".
                  " and c.Cancelled='N' and s.Active='Y' and s.Cancelled='N'";
...
$QueryTable = "$Instrument i join flag f ON (i.CommentID=f.CommentID)".
              " JOIN session s ON (s.ID=f.SessionID) JOIN candidate c".
              " USING (CandID)";
if (isset($InstrumentX)) {
    $QueryTable .= "JOIN flag f2 ON (f2.SessionID=f.SessionID".
                   " AND f2.Test_name=";
    $QueryTable .= "'$InstrumentX') JOIN $InstrumentX i2".
                   " ON (f2.CommentID=i2.CommentID) ";
}


$FullQuery = "SELECT c.PSCID as ID, $FieldX as X, $Field as Y,".
             " s.SubprojectID as Category, c.CandID, s.ID as SessionID,".
             " i.CommentID FROM $QueryTable WHERE $QueryCondition".
             " AND c.CenterID <> 1";
