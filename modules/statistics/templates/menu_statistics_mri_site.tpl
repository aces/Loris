   ...
   <tr bgcolor="#FFFFFF">
      ...
      <td>{foreach from=$data[item].incompletes[$visit] item=Candidate name=CandLoop}
            {if $Candidate.test_url == "PF_Missing"}
                <a href="?test_name=imaging_browser&subtest=viewSession&sessionID={$Candidate.SessionID}">
            {else}
            <a href="?test_name=mri_parameter_form&candID={$Candidate.CandID}&sessionID={$Candidate.SessionID}&commentID={$Candidate.CommentID}">
            {/if}
            {$Candidate.PSCID}</a>
          {/foreach}
      </td>
