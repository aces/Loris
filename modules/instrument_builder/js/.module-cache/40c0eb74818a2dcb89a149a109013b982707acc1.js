QuestionText = React.createClass({displayName: "QuestionText",
	...
	render: function () {
		...
		if (this.props.element.error) {
			errorMessage = (React.createElement("font", {className: "form-error"}, this.props.element.error));
	    ...
	    if(questionText == '' && selected != 'line') {
	        if(selected == 'page-break') {
	        	this.setState(function(state){
					return {
						error: "Must use question text as page header"
					};
				});
	        } else {
	        	this.setState(function(state){
					return {
						error: "No question text specified"
					};
				});
	        }
	        return;
