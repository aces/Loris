...
var DropdownOptions = React.createClass({
  ...
  addOption: function addOption() {
    ...
    if (option === "") {
      var _temp = this.state.error ? this.state.error : {};
      _temp.newSelectOption = "Dropdown options cannot be empty!";
      this.setState({
        error: _temp
      });
      return;
    ...
    if (this.state.error) {
      var _temp2 = this.state.error;
      delete _temp2.newSelectOption;
      this.setState({
        error: _temp2
      });
    }
  ...
  render: function render() {
    ...
    return React.createElement(
      'div',
      null,
      React.createElement(BasicOptions, {
        updateState: this.props.updateState,
        element: this.props.element
      }),
      React.createElement(
...
var DateOptions = React.createClass({
  ...
  render: function render() {
    ...
    var maxYear = this.props.element.Options.MaxDate.split('-')[0];
...
var ListElements = React.createClass({
  ...
  render: function render() {
    return React.createElement(
      ...
      React.createElement(
        'label',
        { htmlFor: 'selected-input', className: 'col-sm-2 control-label' },
        'Question Type:'
      ),
      React.createElement(
        'div',
...
var AddElement = React.createClass({
  ...
  addQuestion: function addQuestion() {
    ...
    var questionText = this.state.Description;
    var questionName = this.state.Name;
    var hasError = false;
    ...
    if (selected === 'date') {
      ...
      var max = this.state.Options.MaxDate;
      ...
      if (minDate > maxDate && min !== '' && max !== '') {
        var _temp3 = this.state.error ? this.state.error : {};

        _temp3.dateOption = "End year append befor start year";
        this.setState({
          error: _temp3
        });
        hasError = true;
      ...
      if (!hasError && this.state.error) {
        var _temp4 = this.state.error;
        delete _temp4.dateOption;
        this.setState({
          error: _temp4
        });
      }
    ...
    if (selected === 'numeric') {
      var _min = this.state.Options.MinValue;
      var _max = this.state.Options.MaxValue;

      if (_min >= _max) {
        var _temp5 = this.state.error ? this.state.error : {};
        _temp5.numeric = "Max value must be larger than min value";
        this.setState({
          error: _temp5
        });
        hasError = true;
      ...
      if (!hasError && this.state.error) {
        var _temp6 = this.state.error;
        delete _temp6.numeric;
        this.setState({
          error: _temp6
        });
      }
    ...
    if (questionText === '' && selected !== 'line') {
      ...
      var _temp7 = this.state.error ? this.state.error : {};
      if (selected === 'page-break') {
        _temp7.questionText = "Must use question text as page header";
      } else {
        _temp7.questionText = "No question text specified";
      }
      this.setState({
        error: _temp7
      });
    ...
    if (!hasError && this.state.error) {
      ...
      // if set
      var _temp8 = this.state.error;
      delete _temp8.questionText;
      this.setState({
        error: _temp8
      });
    }
    ...
    if (questionName === '' && selected !== "header" && selected !== "label" && selected !== 'line' && selected !== 'page-break') {
      ...
      var _temp9 = this.state.error ? this.state.error : {};
      _temp9.questionName = "Must specifiy name for database to save value into";
      this.setState({
        error: _temp9
      });
      hasError = true;
      ...
      var _temp10 = this.state.error;
      delete _temp10.questionName;
      this.setState({
        error: _temp10
      });
    }
  ...
  addOption: function addOption(multi) {
    ...
    this.setState(function (state) {
      ...
      var option = multi ? $("#newmultiSelectOption").val() : $("#newSelectOption").val();
      temp.push(option);
      return {
        options: temp
      };
  ...
  render: function render() {
    ...
    if (this.props.element) {
      buttons = React.createElement('input', {
        className: 'btn btn-default',
        type: 'button',
        value: 'Edit Row',
    ...
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(ListElements, {
