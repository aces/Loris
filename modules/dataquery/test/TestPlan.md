...
This test does this this this and this
...
4. [Recent Queries Tests](#recent-queries-test)
...
Perform the following assertions: (If no queries are available, add some)<br>
   - that queries you made have their parameters correctly displayed (i.e. fields and filters).<br>
   - that `text filter` immediately filter the queries.<br>
   - that clicking `Collapse queries` effectively collapses all queries.<br>
   - Make a mix of the following actions:
      - `Star`: should have a yellow star 
      - `Name` some queries, 
      - `Share` should have a blue shared state 
      - `Rerun` some queries.<br>
   - that using the `Starred Only` checkbox filter only keeps the starred ones.<br>
   - that removing the `Starred Only` checkbox prints the same queries originally printed.<br>
   - Repeat 3.5. and 3.6. for `Shared Only`, `Named Only` and `No run times` checkboxes.<br>
   - Assert that mixing checkboxes returns the right query selection. Only queries that match all of the checked conditions should be displayed.<br>
...
Click the `Pin` icon to perform the following tests (ensure that you have 'admin' permission):
   ...
   - With the `query name` text field empty, click the `Submit` button and
   ...
   Assert that the error message reads :<br>
