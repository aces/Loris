...
/******** B ********/
    ...
    function testViewSessionLinks()
    {
        $this->markTestIncomplete(
            'Forms should be added first, not sure how, AND router.php should be fixed for instruments'
        );
    ...
    */
	...
	// Check that the QC status -VISIT LEVEL- buttons are viewable without _qc permission
	...
	// Check that the QC -VISIT LEVEL- options are viewable with correct permission
	...
	// Test that we can edit the QC status -VISIT LEVEL- with the correct permission 
...
/******** C ********/
    ...
    */
	...
	// Test that we can edit the QC status -IMAGE LEVEL- by changing it from Blank to Pass
	...
	// Check PASS green flag appears next to file name
        ...
        $QCStatusImageCheck = $this->webDriver->findElement(
            WebDriverBy::cssSelector("#image-1 > div > div > div.panel-heading > span.label.label-success")
        )->getText();
	...
	$SelectedLink->click();
        ...
        $this->markTestSkipped(
            'Even if Caveat is set to True, no link is activated --- REDMINE ????'
        );
...
/******** D ********/
    ...
    */
	...
	$SaveButton->click();
        ...
        $this->assertEquals("Testing comment field within Subject header", $SubjectText);
