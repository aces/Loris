  ...
  render() {
    ...
    switch (this.state.fileType) {
      case 'Methylation beta-values':
        inputs.push(
          <FileInput key='fileData'
                     name='fileData'
                     label='File :'
          />
        );
        inputs.push(
          <TextAreaInput key='description'
                         name='description'
                         label='Description :'
          />
        );
...
class FileTypeSelect extends Component {
  ...
  getGenomicFileType() {
      ...
      try {
        switch (xhr.readyState) {
          ...
          case 4:
            let fileType = [
              {genomicFileType: ''},
            ].concat(JSON.parse(xhr.responseText));
    ...
    let url = this.props.baseURL +
      '/AjaxHelper.php?Module=genomic_browser&script=get_genomic_file_type.php';
    xhr.open('POST', url, true);
    xhr.send();
  }
  render() {
    ...
    return (
      <div className='col-xs-12 form-group'>
        <label htmlFor={this.props.name} className='col-xs-3'>
          {this.props.label}<a color='red'><sup> *</sup></a>
        </label>
...
export default GenomicFileUploadModal;
