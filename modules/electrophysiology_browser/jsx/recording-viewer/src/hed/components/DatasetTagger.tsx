...
import {CheckboxElement, SelectDropdown} from '../../ui/Form';
...
/** Dataset tagger component. */
function DatasetTagger({
  activeMenuTab,
  setActiveMenuTab,
  tabsRef,
  filenamePrefix,
  ...
  const getFirstGroupTag = (tag: HEDTag) => {
    const tagsToSearch = applyOverrides([
      ...addedTags,
  ...
  const handleRemoveTag = (tagRelID: any) => {
    if (groupMode) {
      console.warn(
        'If you want to delete a tag, you cannot be in "Group Mode". '
          + 'Press the "Tag Mode" button'
      );
      return;
    }
  ...
  /** Handle HED mouse leave. */
  const handleHEDMouseLeave = () => {
    clearDatasetTooltip();
  };

  /** Clear dataset tooltip. */
    ...
    if (activeFieldValue) { // null between transitions
      hedTags.forEach((hedTag) => {
        ...
        while (pairRelID !== null) {
          const pairRelTag = applyOverrides([
            ...addedTags,
            ...datasetTags[activeColumnName][activeFieldValue],
          ])
            .find((t) => {
          ...
          if (pairRelTag) {
            ...
            console.warn(
              `Something went wrong. Tag with ID ${pairRelID} not found. `
                + 'Should not proceed.'
            );
            pairRelID = null;
          }
  ...
  const buildHEDBadges = (tags: HEDTag[]) => {
    const hedTags = activeMenuTab === 'TAG_MODE'
      ? applyOverrides(tags).filter((tag) => {
        // Filter invalidated structural tags and invalid database entries.
        return tag.HEDTagID !== null || tag.PairRelID !== null;
      })
  ...
  const applyOverrides = (hedTags: HEDTag[]): HEDTag[] => {
    return hedTags.map((hedTag) => {
      const schemaElement = hedTag.schemaElement;
  ...
  const handleUndoGroup = () => {
    const firstTag = applyOverrides(groupedTags)[0];
                                  ...
                                  }}>
                                    {
                                      datasetTags[activeColumnName][activeFieldValue]
                                            ...
                                            const uniqueEndorsements = hedTag.Endorsements
                                              .filter((endorsement) => {
                                                return !endorsements.find(
                                                  (v) => (
                                                    v.EndorsementTime.slice(0, -1)
                                                      === endorsement.EndorsementTime.slice(0, -1)
                                                  && v.EndorsementComment
                                        ...
                                        .map((endorsement) => {
                                          return (
                                            <React.Fragment
                                              key={`endorsement-${endorsement.EndorsedByID}`}
                                            >
                                              {
                      ...
                      <>
                        <div style={{
                          ...
                          )}</span>: <span>(</span>
                          {
                            buildHEDString(
                              [
                                ...applyOverrides(groupedTags),
                                ...getGroupedTagPairings(
                                  applyOverrides(groupedTags)
                                ),
                              ],
                              showLongFormHED
                  ...
                  <>
                    ...
                    }
                    {
                      groupedTags.length === 1 && (
                        applyOverrides(groupedTags)[0].HasPairing == '1'
                        || applyOverrides(groupedTags)[0].PairRelID !== null
                      ) && (
                        <button
                          className='btn btn-primary'
