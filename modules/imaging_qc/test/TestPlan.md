...
12. For one or two candidates, verify that the results are accurate for key columns (`Scan type`, `Scan Done in MRI Parameter Form`, `Tarchive`, `Scan Location`, `QC Status`, `Selected`).
13. Find a scan for which the value of `Scan Done in MRI Parameter Form` is set to `Yes`. Click on the link. Make sure it leads to the appropriate MRI parameter form if and only if the user has permission `data_entry`: if not, the system should display `You do not have access to this page`.
