...
class MriViolationsTestIntegrationTest extends LorisIntegrationTest
{
   ...
   /**
    ...
    /**
     * Tests that, click the link of the problem, the page will go to 
     * mri_protocol_check_violations
     *
     * @return void
     */
    function testNotResolvedProblemLink()
    {
        $this->safeGet($this->url . "/mri_violations/");
        $this->webDriver->findElement(WebDriverBy::Name("PatientName"))
             ->sendKeys("[Test]PatientName");
        $this->webDriver->findElement(WebDriverBy::Name("filter"))->click();
       
        $this->webDriver->findElement(WebDriverBy::linkText("Could not identify scan type"))->click();
        $value = $this->webDriver->findElement(WebDriverBy::cssSelector
                   ("#content > table > tbody > tr > td:nth-child(2) > a"))
               ->getText();
        $this->assertEquals("[Test]PatientName",$value);
    }
}
?>
