!function (modules) {
    ...
    var _AccessProfilePanel = __webpack_require__(1),
        _AccessProfilePanel2 = _interopRequireDefault(_AccessProfilePanel);
    $(function () {
        ReactDOM.render(React.createElement(_AccessProfilePanel2.default, null), document.getElementById("openprofile")), ReactDOM.render(React.createElement(DynamicDataTable, {
    ...
    var _createClass = function () {
        ...
        }(), _Panel = __webpack_require__(2), _Panel2 = _interopRequireDefault(_Panel),
        AccessProfilePanel = function (_React$Component) {
            function AccessProfilePanel(props) {
                ...
                return _this.state = {
                    error: {message: "", className: "alert alert-danger text-center"},
                    PSCID: "",
                    CandID: ""
                }, _this.updateFormElement = _this.updateFormElement.bind(_this), _this.validateAndSubmit = _this.validateAndSubmit.bind(_this), _this
            }

            return _inherits(AccessProfilePanel, _React$Component), _createClass(AccessProfilePanel, [{
                key: "updateFormElement",
                value: function (formElement, value) {
                    var state = this.state;
                    state[formElement] = value, this.setState(state)
                }
            }, {
                key: "validateAndSubmit", value: function () {
                    var state = this.state;
                    return "" === this.state.CandID ? (state.error = {
                        message: "You must enter a DCCID!",
                        className: "alert alert-danger text-center"
                    }, void this.setState(state)) : "" === this.state.PSCID ? (state.error = {
                        message: "You must enter a PSCID!",
                        className: "alert alert-danger text-center"
                    }, void this.setState(state)) : (state.error = {
                        message: "Validating...",
                        className: "alert alert-info text-center"
                    }, this.setState(state), void $.get(loris.BaseURL + "/candidate_list/ajax/validateProfileIDs.php", {
                        CandID: state.CandID,
                        PSCID: state.PSCID
                    }, function (data) {
                        "1" === data ? (state.error = {
                            ...
                            className: "alert alert-info text-center"
                        }, window.location.href = loris.BaseURL + "/" + state.CandID) : state.error = {
                ...
                key: "render", value: function () {
                    var warning;
                    return loris.userHasPermission("access_all_profiles") ? React.createElement("div", null) : ("" !== this.state.error.message && (warning = React.createElement("div", {className: this.state.error.className}, this.state.error.message)), React.createElement("div", {className: "col-sm-3"}, React.createElement(_Panel2.default, {title: "Open Profile"}, React.createElement(FormElement, {
                        name: "openprofile",
                        ...
                        onUserInput: this.updateFormElement
                    }), warning, React.createElement(ButtonElement, {
                        name: "Open Profile",
                        label: "Open Profile",
                        onUserInput: this.validateAndSubmit
                    })))))
