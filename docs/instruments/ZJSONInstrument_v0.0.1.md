...
✗ = an error will be thrown on data entry instructing the user to contact the administrator to resolve the issue. 
...
Default: True. Interaction: AND.

An AND operator makes it necessary that both the rule of the field and the rule of its parent evaluate to true in order for the 
...
Default: False. Interaction: OR.

An OR operator allows either the rule of the field or the rule of its parent to evaluate to true in order for the field to be 
hidden.
...
`options.vertical`: Required boolean. True or false that the radio group is rendered vertically. The default value is false, i.e. 
