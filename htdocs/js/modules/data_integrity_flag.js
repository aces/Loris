...
function changeVisitLabels() {
	...
	var instrument_dropdown_value = getQueryVariable("instrument");
	...
	function(data){
		...
		var i, numInstruments = instruments.length, val, selected = false;
		...
		///Removes the space and replaces the instrument by a space
		$("#instrument option").each(function(n){
			if (this.value == '') {
			     this.remove();
			}
			else if (this.value.match(/Instruments/g)) {
			     $(this).text('');
			}
		});
