...
let _modulesHandleDom = require('./modules/handle-dom');

/*
 * Handy utilities
 */

let _modulesUtils = require('./modules/utils');

/*
 *  Handle sweetAlert's DOM elements
 */

let _modulesHandleSwalDom = require('./modules/handle-swal-dom');

// Handle button events and keyboard events

let _modulesHandleClick = require('./modules/handle-click');

let _modulesHandleKey = require('./modules/handle-key');

let _modulesHandleKey2 = _interopRequireDefault(_modulesHandleKey);

// Default values

let _modulesDefaultParams = require('./modules/default-params');

let _modulesDefaultParams2 = _interopRequireDefault(_modulesDefaultParams);

let _modulesSetParams = require('./modules/set-params');

let _modulesSetParams2 = _interopRequireDefault(_modulesSetParams);
 ...
 */
let sweetAlert; let swal;

exports['default'] = sweetAlert = swal = function() {
  let customizations = arguments[0];
  ...
  let params = (0, _modulesUtils.extend)({}, _modulesDefaultParams2['default']);
  ...
  switch (typeof customizations) {
    ...
    case 'object':
      ...
      for (let customName in _modulesDefaultParams2['default']) {
  ...
  // Modal interactions
  let modal = (0, _modulesHandleSwalDom.getModal)();

  /*
   * Make sure all modal buttons respond to all events
   */
  let $buttons = modal.querySelectorAll('button');
  let buttonEvents = ['onclick', 'onmouseover', 'onmouseout', 'onmousedown', 'onmouseup', 'onfocus'];
  let onButtonEvent = function onButtonEvent(e) {
  ...
  let onKeyEvent = function onKeyEvent(e) {
...
sweetAlert.close = swal.close = function() {
   ...
   */
  let $successIcon = modal.querySelector('.sa-icon.sa-success');
  (0, _modulesHandleDom.removeClass)($successIcon, 'animate');
  (0, _modulesHandleDom.removeClass)($successIcon.querySelector('.sa-tip'), 'animateSuccessTip');
  (0, _modulesHandleDom.removeClass)($successIcon.querySelector('.sa-long'), 'animateSuccessLong');

  let $errorIcon = modal.querySelector('.sa-icon.sa-error');
  (0, _modulesHandleDom.removeClass)($errorIcon, 'animateErrorIcon');
  (0, _modulesHandleDom.removeClass)($errorIcon.querySelector('.sa-x-mark'), 'animateXMark');

  let $warningIcon = modal.querySelector('.sa-icon.sa-warning');
...
sweetAlert.showInputError = swal.showInputError = function(errorMessage) {
  ...
  let $errorIcon = modal.querySelector('.sa-input-error');
  (0, _modulesHandleDom.addClass)($errorIcon, 'show');

  let $errorContainer = modal.querySelector('.sa-error-container');
...
let _handleSwalDom = require('./handle-swal-dom');

let _handleDom = require('./handle-dom');

/*
 * User clicked on "Confirm"/"OK" or "Cancel"
 */
let handleButton = function handleButton(event, params, modal) {
  let e = event || window.event;
  let target = e.target || e.srcElement;

  let targetedConfirm = target.className.indexOf('confirm') !== -1;
  let targetedOverlay = target.className.indexOf('sweet-overlay') !== -1;
  let modalIsVisible = (0, _handleDom.hasClass)(modal, 'visible');
  let doneFunctionExists = params.doneFunction && modal.getAttribute('data-has-done-function') === 'true';

  // Since the user can change the background-color of the confirm button programmatically,
  // we must calculate what the color should be on hover/active
  let normalColor; let hoverColor; let activeColor;
...
var handleCancel = function handleCancel(modal, params) {
  ...
  let functionAsStr = String(params.doneFunction).replace(/\s/g, '');
  let functionHandlesCancel = functionAsStr.substring(0, 9) === 'function(' && functionAsStr.substring(9, 10) !== ')';
...
let addClass = function addClass(elem, className) {
  if (!hasClass(elem, className)) {
    elem.className += ' ' + className;
  }
};

let removeClass = function removeClass(elem, className) {
...
let _show = function _show(elem) {
  elem.style.opacity = '';
  elem.style.display = 'block';
};

let show = function show(elems) {
...
let _hide = function _hide(elem) {
  elem.style.opacity = '';
  elem.style.display = 'none';
};

let hide = function hide(elems) {
  if (elems && !elems.length) {
    return _hide(elems);
  }
  for (let i = 0; i < elems.length; ++i) {
...
let isDescendant = function isDescendant(parent, child) {
...
let getTopMargin = function getTopMargin(elem) {
...
let fadeIn = function fadeIn(elem, interval) {
...
let fadeOut = function fadeOut(elem, interval) {
  interval = interval || 16;
...
let fireClick = function fireClick(node) {
...
let stopEventPropagation = function stopEventPropagation(e) {
...
let handleKeyDown = function handleKeyDown(event, params, modal) {
  let e = event || window.event;
  let keyCode = e.keyCode || e.which;

  let $okButton = modal.querySelector('button.confirm');
  let $cancelButton = modal.querySelector('button.cancel');
  let $modalButtons = modal.querySelectorAll('button[tabindex]');
...
let _defaultParams = require('./default-params');

let _defaultParams2 = _interopRequireDefault(_defaultParams);

/*
 * Add modal + overlay to DOM
 */

let _injectedHtml = require('./injected-html');

let _injectedHtml2 = _interopRequireDefault(_injectedHtml);

let modalClass = '.sweet-alert';
let overlayClass = '.sweet-overlay';

let sweetAlertInitialize = function sweetAlertInitialize() {
  let sweetWrap = document.createElement('div');
 ...
 */
let getModal = function getModal() {
 ...
 */
let getInput = function getInput() {
  let $modal = getModal();
  if ($modal) {
 ...
 */
let getOverlay = function getOverlay() {
 ...
 */
let setFocusStyle = function setFocusStyle($button, bgColor) {
  let rgbColor = (0, _utils.hexToRgb)(bgColor);
 ...
 */
let openModal = function openModal(callback) {
  let $modal = getModal();
  ...
  let timer = $modal.getAttribute('data-timer');

  if (timer !== 'null' && timer !== '') {
    let timerCallback = callback;
    $modal.timeout = setTimeout(function() {
      let doneFunctionExists = (timerCallback || null) && $modal.getAttribute('data-has-done-function') === 'true';
 ...
 */
let resetInput = function resetInput() {
  let $modal = getModal();
  let $input = getInput();
 ...
 */
let fixVerticalPosition = function fixVerticalPosition() {
  let $modal = getModal();
 ...
 */
let alertTypes = ['error', 'warning', 'info', 'success', 'input', 'prompt'];

let setParameters = function setParameters(params) {
  let modal = (0, _handleSwalDom.getModal)();

  let $title = modal.querySelector('h2');
  let $text = modal.querySelector('p');
  let $cancelBtn = modal.querySelector('button.cancel');
  let $confirmBtn = modal.querySelector('button.confirm');
  ...
  if (params.type && !(0, _utils.isIE8)()) {
    let _ret = (function() {
      ...
      let $input = (0, _handleSwalDom.getInput)();
 ...
 */
let hexToRgb = function hexToRgb(hex) {
 ...
 */
let logStr = function logStr(string) {
 ...
 */
let colorLuminance = function colorLuminance(hex, lum) {
  // Validate hex string
