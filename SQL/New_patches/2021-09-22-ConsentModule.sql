...
ALTER TABLE candidate_consent_history
  ADD COLUMN `DirectConsentHistoryID` int(10) unsigned DEFAULT NULL,
  ADD COLUMN `version` DECIMAL(5,2) DEFAULT NULL,
  ADD COLUMN `Comment` varchar(255) DEFAULT NULL,
  ADD CONSTRAINT `FK_candidate_consent_history_direct_consent` FOREIGN KEY (`DirectConsentHistoryID`) REFERENCES `direct_consent_history`(`DirectConsentHistoryID`);
