...
-- Add user & menu permissions for Instrument Builder.  These should be granted/populated in user_perm_rel on a per-user basis
...
-- update Menu permissions for Data Dictionary 
...
--parameter_consent_edit --> add for all users w 'unsend to dcc' permission  
INSERT INTO permissions (code, description, categoryID) VALUES ("candidate_parameter_consent_edit", "Edit Consent status (Candidate Parameters)", 2);
INSERT INTO user_perm_rel (userID, permID) SELECT upr.userID, p2.permID from permissions p, user_perm_rel upr, permissions p2 WHERE p2.code="candidate_parameter_consent_edit" AND  p.code='unsend_to_dcc' and p.permID=upr.permID; 
