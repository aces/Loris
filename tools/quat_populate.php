...
//1)only for the Quat_table_1 populate all the sessions....
...
function GetSelectStatement($parameterType, $field=NULL) {
    ...
    default:
        if($field == null) {
            ...
            } else if($parametterType['SourceField'] == 'Examiner') 
                $field = "e.`full_name`";
            else {
        ...
        if($parameterType['SourceField'] == 'Examiner') 
            $query = "SELECT $field AS Value FROM session s JOIN flag ON (s.ID=flag.SessionID) CROSS JOIN $parameterType[SourceFrom] LEFT JOIN candidate ON (s.CandID = candidate.CandID) LEFT JOIN examiners e ON (e.examinerID=$parameterType[SourceFrom].Examiner) WHERE flag.Administration IN ('All', 'Partial') AND flag.Data_entry='Complete' AND flag.CommentID=$parameterType[SourceFrom].CommentID";
        else {
            $query = "SELECT $field AS Value FROM session s JOIN flag ON (s.ID=flag.SessionID) CROSS JOIN $parameterType[SourceFrom] LEFT JOIN candidate ON (s.CandID = candidate.CandID) WHERE flag.Administration IN ('All', 'Partial') AND flag.Data_entry='Complete' AND flag.CommentID=$parameterType[SourceFrom].CommentID";
        }
        break;
    }
    return $query;
...
//does it for one quat_table at a time..
