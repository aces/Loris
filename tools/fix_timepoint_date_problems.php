 ...
 * run the tool w/ add_instrument option to add missing instruments, and create
 * new feedback
 *
 ...
 *
 * -- when diagnosing a problem (if only CandID is specified,
 * all timepoints are diagnosed
 ...
 * fix_timepoint_date_problems.php diagnose <candID>
 *                          [<sessionID> <newCorrectDate> <screening/visit>]
 *
...
{$argv[0]} help - displays this msg
{$argv[0]} fix_date        <candID> <newDate> <dob/edc>
{$argv[0]} fix_date        <candID> <newDate> <screening/visit> <sessionID>
{$argv[0]} diagnose        <candID>
{$argv[0]} diagnose        <candID> [<newDate> <dob/edc/>]
{$argv[0]} diagnose        <candID> [<newDate> <screening/visit> <sessionID>]
{$argv[0]} add_instrument  <candID> <sessionID> <test_name>
NOTES: The date format is: YYYY-MM-DD.
fix_date: updates the dates in candidate or session table. Does not alter the 
BVL battery
USAGE;
    fwrite(STDERR, $usage);
    return;
...
case 'fix_date':
    ...
    // sessionID
    if (in_array($dateType, array('screening', 'visit'), true)) {
        $sessionID = $argv[5];
    }
    break;
...
switch ($action)
{
  ...
  */
case 'add_instrument':
    // add a missing instrument (sessionID and test name are checked inside
    // the function)
...
case 'fix_date':
   ...
   * also pass the 'correct' date and the date type to see what changes would
   * happen if the dates were different
   */
case 'diagnose':
    if (!empty($sessionID)) {
    ...
    foreach ($listOfTimePoints as $sessionID) {
        ...
        try {
            // wrapping in if statement to avoid PHP notice. Checking for both
            // dateType and newDate at the same time since
...
function addInstrument($sessionID, $testName)
{
    ...
    if (in_array($testName, $existingBattery), true) {
        throw new LorisException(
    ...
    $success = $feedback->createThread(
        'instrument',
        $threadFeedbackType,
        "Instrument ($testName) has been added to the battery. "
        . "You may now complete data entry for this instrument. Please "
        . "respond to this feedback to acknowledge the changes.",
        'Y'
    );
 ...
 * @param string $dateType  type of the date to change
 * @param date   $newDate   the new date in format YYYY-MM-DD
...
function fixDate($candID, $dateType, $newDate, $sessionID=null)
{
    ...
    if (in_array($dateType, array('dob', 'edc'))) {
        ...
        */
        ...
        fwrite(
            STDERR,
            "Updated $dateType to: $newDate, for candidate $candID. Check "
        ...
        if ($dateType == 'visit'
            ...
            throw new LorisException(
                ...
                . "(sessionID: $sessionID) b/c that stage was not started!"
            );
        }
