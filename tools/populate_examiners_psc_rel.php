...
if (!empty($pscCheck)) {
    ...
    foreach ($pscCheck as $k => $row) {
        echo "examiners table contains site ID: ".$row['centerID']
            ." which does not exist in the database (psc).\n";
    }
}
...
if (!empty($certCheck)) {
    ...
    foreach ($certCheck as $k => $row) {
        echo "Certification table contains examiner ID: ".$row['examinerID']
            ." which does not exist in the database (examiners).\n";
    }
}
    ...
    if ($db->tableExists($table) && $db->columnExists($table, 'Examiner')) {
        ...
        if (!empty($instCheck)) {
            ...
            foreach ($instCheck as $k => $row) {
                echo "Instrument $table contains examiner ID: ".$row['Examiner']
                    ." which does not exist in the database (examiners).\n";
            }
        }
    }
...
if (!empty($frrCheck)) {
    ...
    foreach ($frrCheck as $k => $row) {
        echo "Table final_radiological_review contains Final_Examiner IDs: "
            .$row['Final_Examiner']." and ".$row['Final_Examiner2']
    ...
    foreach ($data_array as $k => $dupData) {
        ...
        foreach ($instruments as $table => $name) {
            if ($db->tableExists($table)
                ...
                echo "        -> Updating Instrument $table examinerIDs: "
                    . "$oldExaminerID => $finalExaminerID\n";
                $db->update(
                    $table,
