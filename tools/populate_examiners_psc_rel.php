...
if (!empty($pscCheck)) {
    ...
    foreach ($pscCheck as $k=>$row) {
        echo "examiners table contains site ID: "
            .$row['centerID']." which does not exist in the database (psc).\n";
    }
}
...
if (!empty($pscNullCheck)) {
    ...
    foreach ($pscNullCheck as $k=>$row) {
        echo "examiners table contains NULL sites for examinerID: "
            . "$row[examinerID]\n";
    }
}
...
if (!empty($nameNullCheck)) {
    ...
    foreach ($nameNullCheck as $k=>$row) {
        echo "examiners table contains NULL names for examinerID: "
            . "$row[examinerID]\n";
    }
}
///////// examiners.full_name is null
...
if (!empty($activeNullCheck)) {
    ...
    foreach ($activeNullCheck as $k=>$row) {
        echo "examiners table contains NULL active statuses for examinerID: "
            . "{$row['examinerID']}\n";
    }
}
///////// examiners.full_name is null
$pendingNullCheck = $db->pselect(
    "SELECT * FROM examiners 
    WHERE pending_approval IS NULL OR pending_approval =''",
    array()
);
if (!empty($pendingNullCheck)) {
    $errorPRE =true;
    foreach ($pendingNullCheck as $k=>$row) {
        echo "examiners table contains NULL pending approval statuses for "
...
if (!empty($certCheck)) {
    ...
    foreach ($certCheck as $k=>$row) {
        echo "Certification table contains examiner ID: "
            .$row['examinerID']." which does not exist in the database "
            . "(examiners).\n";
    }
}
    ...
    if ($db->tableExists($table) && $db->columnExists($table, 'Examiner')) {
        ...
        if (!empty($instCheck)) {
            ...
            foreach ($instCheck as $k=>$row) {
                echo "Instrument $table contains examiner ID: "
                    .$row['Examiner']." which does not exist in the database "
                    . "(examiners).\n";
            }
        }
    }
}
///////// final_radiological_review.Final_Examiner2 final_radiological_review.
//Final_Examiner NOT in examiners.examinerID
$frrCheck = $db->pselect(
...
if (!empty($frrCheck)) {
    ...
    foreach ($frrCheck as $k=>$row) {
        echo "Table final_radiological_review contains Final_Examiner IDs: "
            .$row['Final_Examiner']." and "
            .$row['Final_Examiner2']
            ." which do not exist in the database (examiners).\n";
    }
}
...
foreach ($examinersOrganized as $name=>$data_array) {
    ...
    foreach ($data_array as $k=>$dupData) {
        ...
        echo "        -> Updating Certification examinerIDs: "
        ...
        echo "        -> Updating final radiological review examinerIDs: "
            .$oldExaminerID."=>".$finalExaminerID."\n";
        $db->update(
        ...
        foreach ($instruments as $table=>$name) {
            if ($db->tableExists($table)
                && $db->columnExists($table, 'Examiner')
            ) {
...
echo "\n\nThe script ran successfully, make sure to run the clean-up "
    . "commands available in the SQL/Archive/18.0/clean-up directory. \n "
    . "These commands will drop the now redundant fields and add new "
    . "constraints to the examiner tables.\n\n";
