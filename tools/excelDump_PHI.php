        ...
        if (is_file("../project/instruments/NDB_BVL_Instrument_$Test_name.class.inc") || 
            ...
            if(in_array($Test_name, $ddeInstruments)) {
                $extra_fields .= "CASE ddef.Data_entry='Complete' WHEN 1 then 'Y' 
                                  ...
                                  ELSE 'N' 
                                  END AS DDE_Complete, 
                                  CASE WHEN EXISTS (SELECT 'x' FROM conflicts_unresolved cu 
                                  WHERE i.CommentID=cu.CommentId1 OR i.CommentID=cu.CommentId2) 
                                  THEN 'Y' ELSE 'N' END AS conflicts_exist, ";
            }
...
$query = "select distinct c.PSCID, c.CandID, c.Gender, c.DoB, s.SubprojectID from candidate c, session s where c.CandID = s.CandID and substring(c.PSCID, 1, 3) in ('PHI') and c.Active='Y' order by c.PSCID";
