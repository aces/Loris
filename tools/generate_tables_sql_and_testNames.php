...
foreach ($instruments AS $instrument) {
    ...
    foreach ($items AS $item) {
        ...
        case "table":
            ...
            // created first five query entries.
            array_push(
                $sql_query_columns,
            ...
            array_push(
                $sql_query_columns,
                "`Data_entry_completion_status` enum('Incomplete',
            'Complete') NOT NULL default 'Incomplete',\n"
            );
            break;
        case "page":
            $pages[] = $bits[2];
    ...
    foreach ($pages as $pageNo => $page) {
        fwrite(
            ...
            . " VALUES ('$tablename', '" . $tablename  . "_page" . ($pageNo+1)
            . "', '$page');\n"
        );
    }
    fclose($fp);
...
if ($db->isConnected()) {
    ...
    $statement = $db->prepare(
        'REPLACE INTO test_names (Test_name, Full_name, Sub_group) VALUES (?, ?, 1)'
    );
    $statement->bindParam(1, $tablename, PDO::PARAM_STR);
    ...
    foreach ($pages as $pageNo => $page) {
        ...
        $subtest_name = $tablename  . "_page" . ($pageNo+1);
        $statement->bindParam(1, $tablename, PDO::PARAM_STR);
 ...
 * @param string $table   the name used.
 * @param string $name    the table uses.
 *
 * @return string
 */
