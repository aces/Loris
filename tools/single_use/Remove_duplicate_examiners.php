...
$duplicateUIDS = array_keys(
	array_filter(
		$array_count, function($k){ return $k > 1;}
	)
);
$duplicates = array_filter($examiners, function($k) use(&$duplicateUIDS) {
...
if (count($duplicates) === 0) {
	...
	foreach($duplicates as $examinerID => $userID) {
		if (!in_array($examinerID, $correctExaminerIDs)) {
			print_r("Updating examinerID for user $userID ".
				"in certification\n");
			$DB->update(
				'certification',
				[
			...
			print_r("Removing row for user $userID ".
